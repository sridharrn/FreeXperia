...
{
    struct stat info;
    if (0 == stat("/sdcard/xrecovery/.no_confirm", &info))
        return 1;
...
void show_partition_menu()
{
    ...
    if(!device_volumes)
		...
		for (i = 0; i < num_volumes; ++i) {
		    ...
		    else if (strcmp("ramdisk", v->fs_type) != 0 && strcmp("mtd", v->fs_type) == 0 && is_safe_to_format(v->mount_point) && strcmp("/boot", v->mount_point) != 0)
		    {
				sprintf(&format_menue[formatable_volumes].txt, "format %s", v->mount_point);
				format_menue[formatable_volumes].v = &device_volumes[i];
				++formatable_volumes;
...
int run_and_remove_extendedcommand()
{
    ...
    remove("/sdcard/xrecovery/.recoverycheckpoint");
    if (i == 0) {
...
void show_nandroid_advanced_restore_menu()
{
    ...
    char* file = choose_file_menu("/sdcard/xrecovery/backup/", NULL, advancedheaders);
    if (file == NULL)
        return;
        ...
        case 0:
            {
                ...
                if (tmp == NULL)
                {
                    ...
                    sprintf(backup_path, "/sdcard/xrecovery/backup/%d", tp.tv_sec);
                }
                else
                {
                    strftime(backup_path, sizeof(backup_path), "/sdcard/xrecovery/backup/%F.%H.%M.%S", tmp);
                }
...
void process_volumes() {
    ...
    sprintf(backup_path, "/sdcard/xrecovery/backup/%s", backup_name);
...
void handle_failure(int ret)
{
    ...
    if (0 != ensure_path_mounted("/sdcard"))
        return;
    mkdir("/sdcard/xrecovery", S_IRWXU);
    __system("cp /tmp/recovery.log /sdcard/xrecovery/recovery.log");
    ui_print("/tmp/recovery.log was copied to /sdcard/xrecovery/recovery.log. Please open ROM Manager to report the issue.\n");
}
