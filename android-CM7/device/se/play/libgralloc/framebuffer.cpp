...
static void *hdmi_ui_loop(void *ptr)
{
    ...
    while (1) {
        ...
        if (m->pobjOverlay) {
            ...
            property_get("hw.hdmiON", property_value, "0");
            if (!atoi(property_value))
                pTemp->closeChannel();
            else if (atoi(property_value) && !m->videoOverlay) {
                if (!pTemp->isChannelUP()) {
                   if (pTemp->startChannel(m->info.xres, m->info.yres,
                                 m->fbFormat, 1))
                ...
                if (pTemp->isChannelUP()) {
                    ...
                    int height = pTemp->getFBHeight(), x = 0;
                    int fbwidth = m->info.xres, fbheight = m->info.yres;
                    if (fbwidth < fbheight && !(m->orientation)) {
                        x = (width - fbwidth) / 2;
                        if (x < 0)
                            x = 0;
                        if (fbwidth < width)
                    ...
                    else if (fbwidth > fbheight && m->orientation) {
			int t = fbwidth;
			fbwidth = fbheight;
			fbheight = t;
                        x = (width - fbwidth) / 2;
                        ...
                        pTemp->setParameter(OVERLAY_TRANSFORM,
                                           0);
                    pTemp->setPosition(x, 0, width, height);
